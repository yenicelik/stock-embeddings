...
class BaselineModel:
    ...
    def __init__(self, encoder_label, numerical_feature_cols, development=True,regression=False):
        ...
        self.keras_modelcheckpoint_path=self.keras_modelcheckpoint_path+"_keras_dev.hdf5" if development else self.savepath + "_keras.hdf5"
        self.fitted = False
        ...
        numerical_inputs = Input(shape=(len(numerical_feature_cols),), name='num_input')
    ...
    def fit(self, X, y, X_val, y_val, load_model=False):
       ...
       # check_point = ModelCheckpoint('model.hdf5', verbose=True, save_best_only=True)
        ...
        self.keras_model.fit(X, y,validation_data=(X_val, y_val),epochs=3,verbose=0,callbacks=[early_stop, check_point])
        self.fitted = True
...
if __name__ == "__main__":
    ...
    model=BaselineModel(encoder_label,num_feature_cols,development=True)
